...
Notes:
* tests written in Obj-C
* shared/Xcode6 branch
* run in Debug for debugging tests but actually runs production code in Release
* DOCUMENTATION EVEN IN DASH WOOT WOOTY TOOTY!
* `SLKeyboard -typeString:` text can be mashed together and sent to `typeString()` all at once to speed up typing time - tested once and it worked, who knows if it will forever
* `-logElementTree` logs out heirarchy in instruments for any `SLElement`
* 
